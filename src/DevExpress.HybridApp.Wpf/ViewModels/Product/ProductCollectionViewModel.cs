...
namespace DevExpress.DevAV.ViewModels {
    ...
    public partial class ProductCollectionViewModel : CollectionViewModel<Product, ProductInfoWithSales, long, IDevAVDbUnitOfWork> {
        ...
        protected ProductCollectionViewModel(IUnitOfWorkFactory<IDevAVDbUnitOfWork> unitOfWorkFactory = null, UnitOfWorkPolicy unitOfWorkPolicy = UnitOfWorkPolicy.Individual)
            : base(unitOfWorkFactory ?? UnitOfWorkSource.GetUnitOfWorkFactory(), x => x.Products, query => QueriesHelper.GetProductInfoWithSales(query), unitOfWorkPolicy: unitOfWorkPolicy) {
        }
    }
}